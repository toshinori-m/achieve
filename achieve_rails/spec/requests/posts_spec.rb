...
RSpec.describe 'POST /create' , type: :request do
  ...
  context 'ログイン失敗' do
    it '誤ったemailを入力したstatus401が返ってくる' do
      post '/auth/sign_in', params: { email: 'toshi@nori.com', password: user.password }, headers: token
      expect(response.status).to eq (401)
    end

    it '誤ったpasswordを入力したstatus401が返ってくる' do
      post '/auth/sign_in', params: { email: user.email, password: 'toshinori' }, headers: token
      expect(response.status).to eq (401)
    end
  end
  ...
  context 'ログアウト成功' do
    it 'status200が返ってくる' do
      delete '/auth', params: { email: user.email, password: user.password }, headers: token
      expect(response.status).to eq (200)
    end
  end
